...
{
    class StoreTempCommand : ExtruderCommand
    {
        public override MessageType MessageType {get{return MessageType.CMD_STORE_SETTEMPS; }}
        ...
        public StoreTempCommand(UInt16[] Temperatures)
        {
            for(int idx = 0; idx < NUM_HEAT_ZONES; idx++)
            {
        ...
        public override void ParseReponse(byte[] responseBytes)
        {
            ...
            {
                if (responseBytes[MESSAGE_TYPE_OFFSET] == (byte)MessageType.RSP_STORE_SETTEMPS)
                {
